...
const packageName = "@mtayfur/opencode-session-recap";
const previousPackageName = "@mtayfur/opencode-pulse";
...
const previousLocalIndex = current.findIndex(isPreviousLocalEntry);
const publishedIndex = current.findIndex(isPublishedEntry);
const sourceIndex =
  localIndex >= 0 ? localIndex : previousLocalIndex >= 0 ? previousLocalIndex : publishedIndex;
...
function isPublishedEntry(entry: unknown): boolean {
  ...
  return (
    ...
    Boolean(name?.startsWith(`${packageName}@`)) ||
    Boolean(name?.startsWith(`${previousPackageName}@`))
  );
}

function isPreviousLocalEntry(entry: unknown): boolean {
  const name = entryName(entry);
  return Boolean(name?.startsWith("file:") && name.endsWith("/opencode-pulse/dist/index.js"));
}

function isManagedEntry(entry: unknown): boolean {
  const name = entryName(entry);
  return name === localEntry || isPreviousLocalEntry(entry) || isPublishedEntry(entry);
}

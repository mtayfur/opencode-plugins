...
json_set_plugin() {
  ...
  bun -e '
    ...
    const releasePackage = releaseEntry.slice(0, releaseEntry.lastIndexOf("@"));
    ...
    const entryName = p => typeof p === "string" ? p : Array.isArray(p) ? p[0] : undefined;
    const matchesEntry = (p, entry) => entryName(p) === entry;
    const matchesRelease = p => {
      const name = entryName(p);
      return name === releasePackage || name?.startsWith(`${releasePackage}@`);
    };
    const renameEntry = (p, entry) => Array.isArray(p) ? [entry, ...p.slice(1)] : entry;
    const dedupeReleases = (entries, retainedIdx) =>
      entries.filter((p, idx) => idx === retainedIdx || !matchesRelease(p));

    const localIdx = cfg.plugin.findIndex(p => matchesEntry(p, localEntry));
    const releaseIdx = cfg.plugin.findIndex(matchesRelease);

    if (action === "install") {
      if (localIdx >= 0) {
        cfg.plugin = dedupeReleases(cfg.plugin, localIdx);
      ...
      if (releaseIdx >= 0) {
        cfg.plugin[releaseIdx] = renameEntry(cfg.plugin[releaseIdx], localEntry);
        cfg.plugin = dedupeReleases(cfg.plugin, releaseIdx);
        fs.writeFileSync(f, JSON.stringify(cfg, null, 2) + "\n");
      ...
      cfg.plugin[localIdx] = renameEntry(cfg.plugin[localIdx], releaseEntry);
      cfg.plugin = dedupeReleases(cfg.plugin, localIdx);
...
case "${1:-install}" in
  install|--install)
    ...
    if [ -f "${WORKSPACE_ROOT}/bun.lock" ]; then
      bun install --cwd "${WORKSPACE_ROOT}" --frozen-lockfile --filter "@mtayfur/opencode-prompt-enhancer"
